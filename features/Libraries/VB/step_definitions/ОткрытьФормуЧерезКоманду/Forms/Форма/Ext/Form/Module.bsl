...
Процедура ЯОткрываюОсновнуюФормуОбработки(Знач ИмяОбъекта) Экспорт
	ПередОткрытиемОсновнойФормыОбъектаМетаданных("Обработки", ИмяОбъекта);
...
Процедура ЯОткрываюОсновнуюФормуОтчета(Знач ИмяОбъекта) Экспорт
	ПередОткрытиемОсновнойФормыОбъектаМетаданных("Отчеты", ИмяОбъекта);
...
Процедура ЯОткрываюОсновнуюФормуСправочника(Знач ИмяОбъекта) Экспорт
	ПередОткрытиемОсновнойФормыОбъектаМетаданных("Справочники", ИмяОбъекта);
...
Процедура ЯОткрываюОсновнуюФормуДокумента(Знач ИмяОбъекта) Экспорт
	ПередОткрытиемОсновнойФормыОбъектаМетаданных("Документы", ИмяОбъекта);
...
&НаКлиенте
Процедура ПередОткрытиемОсновнойФормыОбъектаМетаданных(ИмяКоллекции, ИмяОбъекта)
	Если ЗначениеЗаполнено(Ванесса.СпецсимволЗаголовкаТекущегоСценария) Тогда
		Если Лев(ИмяОбъекта, 1) = Ванесса.СпецсимволЗаголовкаТекущегоСценария Тогда
			ИмяОбъекта = Сред(ИмяОбъекта, 2);
		Иначе
			АктивноеОкно = Ванесса.ПолучитьГлавноеОкноИзТестовоеПриложение();
			АктивноеОкно.ВыполнитьКоманду("e1cib/command/CommonForm.VA_ВыполнитьДействие.Open");
			АктивноеОкно = Ванесса.ПолучитьАктивноеОкноИзТестовоеПриложение();
			ФормаДействия = АктивноеОкно.НайтиОбъект(Тип("ТестируемаяФорма"), "VA_ВыполнитьДействие");
			ПолеВвода = ФормаДействия.НайтиОбъект(Тип("ТестируемоеПолеФормы"), , "ИмяДействия");
			ПолеВвода.ВвестиТекст("ПолучитьИмяОбъектаМетаданныхПоПредставлению");
			ПолеВвода = ФормаДействия.НайтиОбъект(Тип("ТестируемоеПолеФормы"), , "Параметр1");
			ПолеВвода.ВвестиТекст(ИмяКоллекции);
			ПолеВвода = ФормаДействия.НайтиОбъект(Тип("ТестируемоеПолеФормы"), , "Параметр2");
			ПолеВвода.ВвестиТекст(ИмяОбъекта);
			КнопкаФормы = ФормаДействия.НайтиОбъект(Тип("ТестируемаяКнопкаФормы"), , "ФормаВыполнить");
			КнопкаФормы.Нажать();
			
			ПолеВвода = ФормаДействия.НайтиОбъект(Тип("ТестируемоеПолеФормы"), , "Результат");
			ТекстИмена = ПолеВвода.ПолучитьТекстРедактирования();
			
			КнопкаФормы = ФормаДействия.НайтиОбъект(Тип("ТестируемаяКнопкаФормы"), , "ФормаЗакрыть");
			КнопкаФормы.Нажать();
			
			МассивИмена = Новый Массив;
			Для Инд = 1 По СтрЧислоСтрок(ТекстИмена) Цикл
				МассивИмена.Добавить(СтрПолучитьСтроку(ТекстИмена, Инд));
			КонецЦикла;
				
			//МассивИмена = ПолучитьИмяОбъектаМетаданныхПоПредставлению(ИмяКоллекции, ИмяОбъекта);
			Если МассивИмена.Количество() = 1 Тогда
				ИмяОбъекта = МассивИмена[0];
				
			ИначеЕсли МассивИмена.Количество() = 0 Тогда
				Ванесса.СообщитьПользователю (Ванесса.ПодставитьПараметрыВСтроку(
					Ванесса.Локализовать("Не найден объект метаданных с представлением <%1>"),
					ИмяОбъекта
				));
			...
			Иначе
				Ванесса.СообщитьПользователю (Ванесса.ПодставитьПараметрыВСтроку(
					Ванесса.Локализовать("Обнаружено несколько объектов метаданных с представлением <%1>"),
					ИмяОбъекта
				));
				Ном = 0;
				Для каждого ИмяОбъекта Из МассивИмена Цикл
					Ном = Ном + 1;
					
					Ванесса.СообщитьПользователю (Ванесса.ПодставитьПараметрыВСтроку(
						Ванесса.Локализовать("№%1. Имя=<%2>"),
						Ном,
						ИмяОбъекта
					));
				КонецЦикла;
				...
				ВызватьИсключение Ванесса.ПодставитьПараметрыВСтроку(
					Ванесса.Локализовать("Найдено более одного объекта метаданных с представлением <%1>. Рекомендуется искать объект метаданных по имени."),
					ИмяОбъекта
				);
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьИмяОбъектаМетаданныхПоПредставлению(ИмяКоллекции, Представление)
	Массив = Новый Массив;
	
	Для Каждого ОбъектМетаданных Из Метаданные[ИмяКоллекции] Цикл
		Если ОбъектМетаданных.Представление() = Представление Тогда
